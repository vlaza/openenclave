...
void oe_load_quote_provider()
{
    if (provider.handle == 0)
    {
        ...
        HMODULE _handle =
            LoadLibraryEx("dcap_quoteprov.dll", NULL, 0);
        if (_handle != 0)
        {
            provider.get_revocation_info = (sgx_ql_get_revocation_info_t)
                GetProcAddress(_handle, SGX_QL_GET_REVOCATION_INFO_NAME);
            provider.free_revocation_info = (sgx_ql_free_revocation_info_t)
                GetProcAddress(_handle, SGX_QL_FREE_REVOCATION_INFO_NAME);
            ...
            provider.get_qe_identity_info = (sgx_get_qe_identity_info_t)
                GetProcAddress(_handle, SGX_QL_GET_QE_IDENTITY_INFO_NAME);
            provider.free_qe_identity_info = (sgx_free_qe_identity_info_t)
                GetProcAddress(_handle, SGX_QL_FREE_QE_IDENTITY_INFO_NAME);

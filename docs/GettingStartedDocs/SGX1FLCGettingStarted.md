...
```bash
openenclave/scripts/ansible/install-ansible.sh
```
...
```bash
ansible-playbook openenclave/scripts/ansible/deploy_for_oe_devs_acc.yml
```

If you are not running in an ACC VM, you should instead run:

```bash
ansible-playbook openenclave/scripts/ansible/deploy_for_oe_devs.yml
```
